...
wafepaApp.controller("EditActivityCtrl", function($scope, $http, $routeParams,
		$location) {
	...
	var getActivity = function() {
		...
		var promise = $http.get(url);
		promise.then(function uspeh(odgovor) {
			$scope.activity = odgovor.data;
		}, function neuspeh(odgovor) {
...
wafepaApp.controller("AddActivityCtrl", function($scope, $http, $location) {
	...
	$scope.doAdd = function() {
		...
		$http.post(url, $scope.activity).then(function success() {
			$location.path("/activities")
		}, function error() {
			alert("Couldn't save the activity")
		})
