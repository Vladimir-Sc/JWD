...
//import doed.l9.wp.servlets.example04.Osoba;
...
public class Login extends HttpServlet {
	...
	private ArrayList<User> autorizovani = null;
	...
	public void init(ServletConfig config) throws ServletException {
		...
		super.init(config);
		autorizovani = new ArrayList<User>();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (User temp: users) {
			...
			if (temp.getUsername().equals(username) && temp.getPassword().equals(password)) {
				...
				response.sendRedirect("WebShopServlet");
				autorizovani.add(user);
				getServletContext().setAttribute("autorizovani", autorizovani);
				return;
			}
